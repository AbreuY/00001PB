...
//		}
		...
		float radius;
		if (w > h) {
			radius = h * mDrawableRadius;
		} else {
			radius = w * mDrawableRadius;
		}

		myPolyGone.setPolygon(x, y, radius, 4);
		
		canvas.drawPath(myPolyGone.getPath(), mBorderPaint);
		//canvas.drawPath(myPolyGone.getPath(), mBorderPaint);
	}
